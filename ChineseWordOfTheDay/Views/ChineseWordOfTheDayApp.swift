...
struct ChineseWordOfTheDayApp: App {
    ...
    @StateObject private var wordDisplayViewModel = WordDisplayViewModel(context: PersistenceController.shared.context)
...
extension ChineseWordOfTheDayApp {
    var body: some Scene {
        WindowGroup {
            Group {
                if loadingManager.isAppLoading {
                    LoadingContainerView(loadingManager: loadingManager)
                        .transition(.opacity)
                } else {
                    ContentView()
                        .environment(\.managedObjectContext, PersistenceController.shared.context)
                        .environmentObject(wordDisplayViewModel)
                        .onAppear {
                            // Set the delegate for CloudKit notifications
                            appDelegate.delegate = wordDisplayViewModel
                        }
                        .transition(.opacity)
                }
            }
            .animation(.easeInOut(duration: 0.5), value: loadingManager.isAppLoading)
        }
    }
